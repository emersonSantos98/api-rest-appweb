...
class LabelService {
    ...
    async generateLabel(productId, labelCountPerVariation = 140) {
        try {
    ...
    async generateETickets(product, labelCountPerVariation) {
        const { name_product, total_cost, variations } = product;
        ...
        const margin = 10;
        const horizontalSpacing = (pageWidth - (labelsPerRow * labelWidth) - 2 * margin) / (labelsPerRow - 1);
        const verticalSpacing = (pageHeight - (labelsPerColumn * labelHeight) - 2 * margin) / (labelsPerColumn - 1) - 2;
        ...
        for (const variation of variations) {
            ...
            for (let i = 0; i < labelCountPerVariation; i++) {
                const { color, size, barcode } = variation;
                const qrCodeData = `${name_product} - ${color} - ${size} - ${total_cost} - ${barcode}`;
                ...
                currentPage.drawImage(qrCodeImage, { x: currentX, y: currentY, width: labelWidth, height: labelHeight });
        ...
        const pdfPath = path.resolve(__dirname, 'labels.pdf');
